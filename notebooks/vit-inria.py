    ...
    def __getitem__(self, idx):
        ...
        for i, f in enumerate(batch_files):
            try:
                ...
                angle = np.random.choice([0.0, 22.5, 45.0, 67.5, 90.0, 112.5, 135.0, 157.5, 180.0, 202.5, 
...
# train
model.compile(loss=angular_loss_mae, optimizer=Adadelta(learning_rate=0.01), metrics=[tf_custom_accuracy])
model.load_weights("/data/subhadip/weights/model-vit-ang-loss.h5")
# acc = Accuracy(batch_size=16)
train_gen = RotGenerator("/data/subhadip/INRIA/jpg/", 4, IMAGE_SIZE, is_vit=True)
...
cp = ModelCheckpoint("/data/subhadip/weights/model-vit-ang-loss-inria.h5", save_weights_only=False, 
